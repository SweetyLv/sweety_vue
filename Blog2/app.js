...
app.use( function(req, res, next) {
    ...
    //console.log(typeof req.cookies.get('userInfo'));//服务,类型是string，需要解析成对象
...
mongodb.connect('mongodb://localhost:27018/blog', function() {
          console.log('数据库连接成功');
          app.listen(8081);
    }
);*/
...
/*var promise = mongoose.createConnection('mongodb://localhost:27018/blog', {
    useMongoClient: true
});
promise.then(function(db) {
   connection.openUri('mongodb://localhost:27018/blog',function(err) {
        if (err) {
            console.log('数据库连接失败');
        } else {
            console.log('数据库连接成功');
